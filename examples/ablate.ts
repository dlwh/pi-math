...
interface Result {
  seed:number;mode:string;status:string;rounds:number;checkedNondegenerate:number;distinctConjectures:number;visibleAtEnd:number;
  trajectory:{round:number;statement:string;featureCount:number;priors:Controls["priors"];proof:string;counterexamples:string[];nondegenerate:boolean;weightsBefore:Patch[];weightsAfter:Patch[]}[];
}
const results:Result[]=[];
for(const seed of [11,29,47,71,101])for(const mode of modes){
  ...
  for(const patch of data.patches)patch.weights=patch.weights.map(w=>w===0?0:0.5+random()/2);
...
const out=resolve(process.argv[2]??"test-output/ablations");await mkdir(out,{recursive:true});
...
await writeFile(join(out,"report.md"),md);console.log(md);
