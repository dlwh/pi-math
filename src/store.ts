...
export class ObjectStore {
    ...
    async initialize(): Promise<void> {
        for (const path of [join(resolve(this.cwd), ".pi"), this.root, join(this.root, "objects")]) {
            ...
            if ((await lstat(path)).isSymbolicLink())
                throw new Error("Research storage cannot use a symlink");
        }
        const actual = await realpath(this.root), base = await realpath(this.cwd);
        if (!actual.startsWith(base + "/") && !actual.startsWith(base + "\\"))
            throw new Error("Research storage escaped the project");
    }
    ...
    async get(key: string): Promise<unknown> {
        ...
        if (stat.isSymbolicLink())
            throw new Error("Research object cannot be a symlink");
        if (stat.size > 32000000)
            throw new Error("Research object exceeds size limit");
        const text = await readFile(file, "utf8");
        if (text.length > 32000000)
            throw new Error("Research object exceeds size limit");
        const value: unknown = JSON.parse(text);
        if (hash(value) !== key)
            throw new Error("Corrupt research object: hash mismatch");
        return value;
    }
    ...
    async restore(entries: readonly BranchEntry[]): Promise<ResearchState | null> {
        ...
        if (ref?.version !== 1 || typeof ref.hash !== "string")
            throw new Error("Unsupported research checkpoint");
        const state = StateSchema.parse(await this.get(ref.hash));
        if (state.id !== ref.runId)
