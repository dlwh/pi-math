...
import { Broker, seededRandom, sample } from "./inference.ts";
...
export class StageRunner {
  ...
  async run<T>(stage: string, instruction: string, input: unknown, schema: z.ZodType<T>, options: TreeOptions<T> = {}): Promise<Bundle<T>> {
    ...
    const review = async (id: string, value: T, inherited: Objection[], proposedResolutions: {id:string;reason:string}[], parents: string[]): Promise<Bundle<T>> => {
      ...
      const reviews = await Promise.all(Array.from({length:this.config.reviewers}, async (_,i) => {
        ...
        if (judgment.resolved.some(r=>!allowed.has(r.id))) throw new Error("Reviewer attempted to resolve an unknown objection");
      ...
      for (const r of reviews) {
        for (const [i,issue] of r.review.issues.entries()) objections.push({...issue,id:`${r.id}-issue-${i}`,source:r.id});
        if (r.review.verdict !== "accept" && !r.review.issues.some(o=>o.severity!=="minor")) objections.push({id:`${r.id}-verdict`,source:r.id,claim:"Reviewer did not accept this artifact",reason:r.review.summary,severity:"major",scope:"artifact",sections:[]});
      }
    ...
    for (let level=1;level<this.config.widths.length;level++) {
      ...
      population = await Promise.all(groups.map(async (group,i) => {
        ...
        if (result.resolutions.some(r=>!inherited.some(o=>o.id===r.id))) throw new Error("Aggregator referenced an unknown objection");
