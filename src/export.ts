...
export function summary(w: Workbench): string {
    ...
    if (p) {
        ...
        if (p.failure)
            parts.push(`Last interruption: ${p.failure}`);
        if (p.strategy)
            parts.push(`Route: ${p.strategy.value.mechanism}`);
        if (p.gate)
            parts.push(`Readiness: ${p.gate.value.reason}`);
        if (p.plan)
            parts.push("Sections:", ...p.plan.sections.map(s => `- ${s.id}: ${s.title} — ${p.sections[s.id]?.status ?? "pending"}; depends on ${s.dependsOn.join(", ") || "none"}`));
        const objections = [...(p.strategy?.objections ?? []), ...(p.gate?.objections ?? []), ...(p.verification?.objections ?? [])];
        if (objections.length)
            parts.push("Open objections:", ...objections.map(o => `- [${o.severity}] ${o.id}: ${o.claim} — ${o.reason}`));
        if (p.verification)
            parts.push(`Global model review: ${p.verification.value.verdict} — ${p.verification.value.summary}`);
        ...
        parts.push("No proof project. Start with /math start <question>.");
    if (d) {
        parts.push(`Discovery: ${d.status}; ${d.rounds.length}/${d.maxRounds} rounds; ${d.dataset.rows.length} rows`);
        const last = d.rounds.at(-1);
        if (last)
            parts.push(`Conjecture: ${formatExpr(last.conjecture)}`, `Evidence: ${last.proof.outcome}; ${last.evidence.counterexamples.length} counterexamples; ${last.evidence.nondegenerate ? "nondegenerate" : "blocked by " + last.evidence.reasons.join(", ")}`);
    }
...
export async function exportWorkbench(store: ObjectStore, w: Workbench, write: (path: string, text: string) => Promise<void> = async (path, text) => { await writeFile(path, text, { flag: "wx", mode: 0o600 }); }): Promise<string> {
    ...
    const dir = await mkdtemp(join(store.root, "export-"));
    ...
    await write(join(dir, "research.md"), markdown(w));
    await write(join(dir, "proof.tex"), latex(w));
    return dir;
}
