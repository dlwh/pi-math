...
export class PiWorker implements Worker {
    ...
    async complete(request: WorkerRequest): Promise<WorkerResponse> {
        ...
        if (!model)
            throw new Error("No research model available; select a Pi model or correct the role configuration");
        if (this.ctx.scopedModels.length && !this.ctx.scopedModels.some(s => s.model.id === model.id && s.model.provider === model.provider))
            throw new Error("Research model is outside the session's scoped models");
        ...
        if (response.stopReason !== "stop")
            throw new Error(`Research worker ended with ${response.stopReason}: ${response.errorMessage ?? "No complete response"}`);
        if (response.content.some(c => c.type === "toolCall"))
            throw new Error("Tool calls are not allowed in isolated research workers");
