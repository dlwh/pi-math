...
export function regressionLoss(expr:Expr,accuracy:number,priors:Controls["priors"],alpha=3):number{
  if(!Number.isFinite(accuracy)||accuracy<0||accuracy>1)throw new Error("Invalid empirical accuracy");
  let prior=0;
  function visit(e:Expr){if(e.kind==="int"||e.kind==="var")return;prior+=priors[e.kind]??0;if(e.kind==="not")visit(e.arg);else{visit(e.left);visit(e.right);}}
  visit(expr);return Math.exp(alpha*(1-accuracy))-prior+size(expr)*0.002;
}
/** Bounded enumerative symbolic regression; a baseline, not PySR or a trained policy. */
export function spotSymbolically(input:FeatureInput):{expression:Expr;description:string}[]{
  ...
  outer:for(const kind of ops)for(const a of input.features.map(v))for(const b of input.features.map(v)){
    terms.push(op(kind,a,b));if(terms.length>=limit)break outer;
  }
  const pairs=terms.slice(leaves.length);
  outer:for(const pair of pairs)for(const leaf of input.features.map(v))for(const kind of ['add','sub']as const){
    terms.push(op(kind,pair,leaf));if(terms.length>=limit)break outer;
  }
  const candidates=new Map<string,{expression:Expr;score:number}>();
  ...
  if(!mass)throw new Error("No visible data for regression");
  for(const term of terms){
    ...
    for(const target of targets){
      ...
      if(certify(expression,[]))continue; // Arithmetic identities do not constitute discovered features.
      const key=canonical(expression),score=regressionLoss(expression,accuracy,input.controls.priors);
      if(!candidates.has(key)||score<candidates.get(key)!.score)candidates.set(key,{expression,score});
    }
  }
  const ranked=[...candidates.values()].sort((a,b)=>a.score-b.score||canonical(a.expression).localeCompare(canonical(b.expression)));
  if(!ranked.length)throw new Error("No supported symbolic feature found; change the data, vocabulary or model policy");
...
export class HeuristicPolicy implements Policy{
  ...
  async scaffold(input:Parameters<Policy["scaffold"]>[0]):ReturnType<Policy["scaffold"]>{
    ...
    const best=ranked[0];if(!best)throw new Error("No atoms to scaffold");
    // Try a supported, nonvacuous relation between distinct features before falling back to one atom.
    if(input.feedback?.rho===0)for(const a of ranked)for(const b of ranked){
      if(a.atom.id===b.atom.id||canonical(a.atom.expression)===canonical(b.atom.expression))continue;
      const expression=op("implies",a.atom.expression,b.atom.expression);
      if(certify(expression,[]))continue;
      if(!input.rows.some(r=>evaluate(a.atom.expression,r.values)===true))continue;
      if(weightedAccuracy(expression,input.rows)>best.accuracy)return {expression:{kind:"implies",left:{kind:"atom",id:a.atom.id},right:{kind:"atom",id:b.atom.id}},reason:"Relate local features under an explicit observed hypothesis"};
    }
  ...
  async skeptic(input:Parameters<Policy["skeptic"]>[0]){
    const updates:{patch:string;row:string;weight:number}[]=[];
    for(const row of input.evidence.counterexamples){
      const i=input.dataset.rows.findIndex(r=>r.id===row);
      for(const patch of input.dataset.patches)if(patch.weights[i]!==1&&updates.length<input.maxChanges)updates.push({patch:patch.id,row,weight:1});
    }
    // If no refutation is available, widen exposure rather than announcing correctness.
    if(!updates.length)for(const patch of input.dataset.patches)for(const [i,weight]of patch.weights.entries())if(weight===0&&updates.length<input.maxChanges)updates.push({patch:patch.id,row:input.dataset.rows[i]!.id,weight:1});
