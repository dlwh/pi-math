...
export function leanSource(expr:Expr,premises:readonly Expr[]):string{
  ...
  function render(e:Expr):string{
    if(e.kind==="int")return `(${e.value} : Int)`;if(e.kind==="var")return index.get(e.name)!;
    if(e.kind==="not")return `(¬ ${render(e.arg)})`;
...
export interface ProcessResult {stdout:string;stderr:string;exitCode:number|null;durationMs:number;problem:string|null}
export async function executeBounded(command:string,args:readonly string[],cwd:string,timeoutMs:number,signal?:AbortSignal,maxBytes=128_000):Promise<ProcessResult>{
  if(signal?.aborted)throw new Error("Research cancelled");
  return new Promise(resolve=>{
    const start=Date.now();let stdout="",stderr="",bytes=0,problem:string|null=null,finished=false;
    ...
    const finish=(exitCode:number|null)=>{if(finished)return;finished=true;clearTimeout(timer);signal?.removeEventListener("abort",cancel);resolve({stdout,stderr,exitCode,durationMs:Date.now()-start,problem});};
    const stop=(message:string)=>{problem??=message;kill();};
    const timer=setTimeout(()=>stop("Process timed out"),timeoutMs);
    const cancel=()=>stop("Research cancelled");signal?.addEventListener("abort",cancel,{once:true});
    for(const [stream,key]of [[child.stdout,"stdout"],[child.stderr,"stderr"]]as const)stream.on("data",(chunk:Buffer)=>{
      const available=Math.max(0,maxBytes-bytes);bytes+=chunk.length;
      const text=chunk.subarray(0,available).toString("utf8");if(key==="stdout")stdout+=text;else stderr+=text;
      if(bytes>maxBytes)stop("Process output limit exceeded");
    });
    child.on("error",error=>{problem=error.message;finish(null);});child.on("close",code=>finish(code));
  });
}
export function cleanLeanAxioms(stdout:string):boolean{
  if(/sorryAx|\bsorry\b|\badmit\b/.test(stdout))return false;
  if(/piMathClaim[^\n]*does not depend on any axioms/.test(stdout))return true;
...
export class LeanProver implements Prover{
  constructor(readonly project:string,readonly timeoutMs=30_000,readonly executable="lake"){}
  async prove(expr:Expr,premises:readonly Expr[],signal?:AbortSignal):Promise<ProofFeedback>{
    ...
    try{
      const file=join(dir,"Claim.lean");await writeFile(file,source,{mode:0o600});
