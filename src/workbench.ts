...
const CertificateTreeSchema:z.ZodType<CertificateTree>=z.lazy(()=>z.union([
  z.object({kind:z.literal("linear"),weights:z.array(z.string().max(10_000)).max(160)}).strict(),
...
export const SettingsSchema=z.object({
  ...
  lean:z.object({project:z.string().min(1),timeoutMs:z.number().int().min(100).max(300_000).default(30_000)}).strict().nullable().default(null),
}).strict();
...
export interface Workbench{
  version:1;proof:ResearchState|null;discovery:DiscoveryState|null;settings:Settings;
  audit:string[];artifacts:string[];evidence:string[];pastRuns:string[];
  humanEvents:{action:string;reason:string;at:string;artifactHash:string}[];
}
...
export function parseWorkbench(raw:unknown):Workbench{
  ...
  if(w.discovery){
    const d=w.discovery;parseDataset(d.dataset);
    for(const round of d.rounds){parseExpr(round.conjecture,d.dataset.features);round.premises.forEach(p=>parseExpr(p,d.dataset.features));checkedFeedback(round.conjecture,round.premises,round.proof);}
    if(d.status==="candidate-found"&&!(d.rounds.at(-1)?.proof.rho===1&&d.rounds.at(-1)?.evidence.nondegenerate))throw new Error("Invalid discovery termination claim");
  }
  return w;
}
...
export async function restoreWorkbench(store:ObjectStore,branch:readonly BranchEntry[]):Promise<Workbench>{
  ...
  if(!entry)return emptyWorkbench();
