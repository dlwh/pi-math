...
import { z } from "zod";
import {
  StateSchema, ConfigSchema, StrategySchema, GateSchema, PlanSchema, SectionSchema,
  VerificationSchema, RevisionSchema, KnowledgeSchema, serious,
  type ResearchState, type Config, type Knowledge, type ProofPlan,
} from "./schema.ts";
...
export function draft(state: ResearchState): string {
  ...
  return `# ${state.plan.title}\n\n${state.plan.abstract}\n\n` + state.plan.sections.map((s,i)=>
    `## ${i+1}. ${s.title} [${s.id}]\n\n${state.sections[s.id]?.candidate?.value.body ?? `[Unfinished: ${s.task}]`}\n`,
  ).join("\n");
}
export function archive(state: ResearchState): void {
  if (state.strategy || state.plan) state.archives.push({round:state.round,problem:state.problem,draft:draft(state),strategy:structuredClone(state.strategy),verification:structuredClone(state.verification)});
}
export function humanDecision(state: ResearchState, kind: ResearchState["decisions"][number]["kind"], reason: string): void {
  if (!reason.trim()) throw new Error("A human decision needs a reason");
...
export function approve(state: ResearchState, reason: string): void {
  if (state.phase === "awaiting-route") { humanDecision(state,"route",reason); state.phase="decompose"; }
  else if (state.phase === "awaiting-plan") { humanDecision(state,"plan",reason); state.phase="solve"; }
  else if (state.phase === "awaiting-acceptance") { humanDecision(state,"accept",reason); state.phase="accepted"; }
  else throw new Error("There is no current artifact awaiting human approval");
  state.revision++;
}
export function reexplore(state: ResearchState, reason: string): void {
  archive(state); humanDecision(state,"reexplore",reason);
  state.phase="explore";state.plan=null;state.sections={};state.verification=null;state.gate=null;state.failure=null;state.revision++;
}
export function amend(state: ResearchState, problem: string, reason: string): void {
  if (!problem.trim()) throw new Error("Empty target");
  reexplore(state,reason);humanDecision(state,"amend",reason);
  state.problem=problem;state.strategy=null;state.round=0;
}
export function addKnowledge(state: ResearchState, knowledge: Knowledge): void {
  KnowledgeSchema.parse(knowledge);
  if (state.knowledge.some(k=>k.id===knowledge.id)) throw new Error("Knowledge IDs are immutable; add a new entry with provenance");
  state.knowledge.push(knowledge);state.revision++;
}
...
export class ProofEngine {
  ...
  private async curate(start: number): Promise<void> {
    ...
    for(const entry of result.entries){
      if(entry.sources.some(s=>!ids.has(s))) throw new Error("Curator cited an unknown artifact");
  ...
  async step(): Promise<ResearchState> {
    ...
    if (["awaiting-route","awaiting-plan","awaiting-acceptance","accepted","blocked"].includes(s.phase)) return s;
    s.failure=null;
    ...
    try {
      switch(s.phase){
        case "explore": {
          ...
          s.gate=null;s.plan=null;s.sections={};s.verification=null;s.phase="gate";
          await this.curate(start);break;
        }
        case "gate": {
          if(!s.strategy)throw new Error("No strategy to assess");
          ...
          if(ready)s.phase="awaiting-route";
          else {archive(s);s.phase="explore";}
          break;
        }
        case "decompose": {
          ...
          if(serious(plan.objections))throw new Error("Proof plan has unresolved material objections");
          s.plan=plan.value;s.sections=Object.fromEntries(s.plan.sections.map(t=>[t.id,emptyProgress()]));s.phase="awaiting-plan";break;
        }
        case "solve": {
          if(!s.plan)throw new Error("No proof plan");
          const eligible=frontier(s.plan,s.sections);
          if(!eligible.length)throw new Error("No eligible proof section");
          const exhausted=eligible.filter(id=>s.sections[id]!.attempts>=s.config.maxSectionAttempts);
          if(exhausted.length){s.phase="blocked";s.failure=`Local retry limit: ${exhausted.join(", ")}`;break;}
          const result=await Promise.allSettled(eligible.map(async id=>{
            const task=s.plan!.sections.find(t=>t.id===id)!;
            const progress=s.sections[id]!;progress.attempts++;
          ...
          for(const [i,r] of result.entries())if(r.status==="rejected"){
            const p=s.sections[eligible[i]!]!;p.status="failed";p.failure=String(r.reason);
          }
          if(Object.values(s.sections).every(p=>p.status==="accepted"))s.phase="verify";
          if(result.some(r=>r.status==="rejected"))throw new Error("One or more section workers failed; completed independent sections were preserved");
          break;
        }
        case "verify": {
          if(!s.plan||!Object.values(s.sections).every(p=>p.status==="accepted"))throw new Error("Cannot verify an incomplete proof");
          ...
          if(clean)s.phase="awaiting-acceptance";
          else {archive(s);s.phase=central||v.verdict==="reexplore"?"explore":"revise";}
          await this.curate(start);break;
        }
        case "revise": {
          if(!s.plan||!s.verification)throw new Error("Missing revision context");
          const revision=await this.runner.run("revise",PROMPTS.revise,{...this.context(),document:draft(s)},RevisionSchema);
          if(serious(revision.objections))throw new Error("Revision plan has unresolved material objections");
          const r=revision.value;
          if(r.action==="reexplore"){s.phase="explore";break;}
          const known=new Set(s.plan.sections.map(t=>t.id));
          if(r.affected.some(id=>!known.has(id)))throw new Error("Revision references an unknown section");
          ...
          if(!affected.length)affected.push(...known); // Whole-document concern: conservatively invalidate all.
          ...
          s.sections=reviseGraph(s.plan,next,s.sections,affected);s.plan=next;
          s.phase=r.action==="outline"?"awaiting-plan":"solve";break;
        }
      }
    } catch(error) {
      s.failure=error instanceof Error?error.message:String(error);throw error;
    } finally {
      s.revision++;StateSchema.parse(s);await this.checkpoint(s);
    }
    return s;
  }
  async run(): Promise<ResearchState> {
    while(!["awaiting-route","awaiting-plan","awaiting-acceptance","accepted","blocked"].includes(this.state.phase))await this.step();
    return this.state;
  }
}
