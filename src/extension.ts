...
export async function readProjectJson(cwd: string, path: string): Promise<unknown> {
    ...
    if (!cleaned)
        throw new Error("Supply a JSON file path");
    const base = await realpath(cwd), file = await realpath(resolve(cwd, cleaned)), rel = relative(base, file);
    if (rel === ".." || rel.startsWith("../") || rel.startsWith("..\\") || isAbsolute(rel))
        throw new Error("Input JSON must be inside the current project");
    if ((await stat(file)).size > 2000000)
        throw new Error("Input JSON exceeds 2 MB");
    return JSON.parse(await readFile(file, "utf8"));
}
export default function mathExtension(pi: ExtensionAPI): void {
    let w: Workbench = emptyWorkbench(), store: ObjectStore | null = null, loaded = false, epoch = 0, loading: Promise<void> | null = null;
    ...
    async function load(ctx: ExtensionContext) {
        if (loading)
            await loading;
        if (loaded && store?.cwd === ctx.cwd) {
            if (restoreFailure)
                throw new Error(restoreFailure);
            return;
        }
        const destination = new ObjectStore(ctx.cwd), branch = ctx.sessionManager.getBranch();
        store = destination;
        restoreFailure = null;
        const job = (async () => {
            try {
                w = await restoreWorkbench(destination, branch);
                const entry = [...branch].reverse().find(e => e.type === "custom" && e.customType === WORKBENCH_TYPE);
                lastCheckpoint = entry ? join(destination.root, "objects", `${(entry as {
                    data: {
                        hash: string;
                    };
                }).data.hash}.json`) : null;
                ...
                restoreFailure = `Cannot restore research checkpoint: ${String(error)}`;
                throw new Error(restoreFailure);
            }
            finally {
                loaded = true;
    ...
    async function save(ctx: ExtensionContext, expectedEpoch = epoch): Promise<void> {
        ...
        const pending = saveTail.then(async () => {
            if (expectedEpoch !== epoch || !destination)
                throw new Error("Research session changed; refusing a stale write");
            parseWorkbench(snapshot);
            const key = await destination.put(snapshot);
            if (expectedEpoch !== epoch)
                throw new Error("Research session changed during checkpoint");
            pi.appendEntry(WORKBENCH_TYPE, { version: 1, hash: key });
            lastCheckpoint = join(destination.root, "objects", `${key}.json`);
            status(ctx);
        });
    ...
    async function exclusive<T>(ctx: ExtensionContext, work: (signal: AbortSignal) => Promise<T>, signal?: AbortSignal): Promise<T> {
        if (running)
            throw new Error("Research is already running; use /math cancel first");
        if (!ctx.isProjectTrusted())
            throw new Error("Trust this project in Pi before starting research work");
        controller = new AbortController();
        const combined = signal ? AbortSignal.any([signal, controller.signal]) : controller.signal;
        // Install the busy marker before any worker or disk await.
        const job = Promise.resolve().then(async () => { await load(ctx); if (combined.aborted)
            throw new Error("Research cancelled"); return work(combined); });
        running = job;
        status(ctx);
        try {
            return await job;
    ...
    async function stop() { controller?.abort(); if (running)
        await running.catch(() => { }); if (loading)
        await loading.catch(() => { }); await saveTail; }
    function result(text = summary(w)) { return { content: [{ type: "text" as const, text: text.length > 12000 ? text.slice(0, 12000) + `\n[Truncated. Full state: ${lastCheckpoint ?? "use /math export"}]` : text }], details: { checkpoint: lastCheckpoint } }; }
    ...
    async function replaceProof(ctx: ExtensionContext, problem: string, assumptions: string[] = []) {
        if (w.proof)
            w.pastRuns.push(await store!.put(w.proof));
        w.proof = newResearch(problem, assumptions, w.settings.inference);
        await save(ctx);
    }
    async function proofWork(ctx: ExtensionContext, signal: AbortSignal, all: boolean) {
        if (!w.proof)
            throw new Error("Start a proof project first");
        ...
        try {
            ...
            ctx.ui.notify(`Research used ${calls.usage.calls} model calls; reported cost $${calls.usage.reportedCost.toFixed(4)}`, "info");
        }
    }
    ...
    async function discover(ctx: ExtensionContext, signal: AbortSignal, all: boolean) {
        if (!w.discovery)
            throw new Error("Load a discovery dataset first");
        ...
        do {
            await engine.step();
            await save(ctx);
        } while (all && w.discovery.status === "active");
    }
    async function importDataset(ctx: ExtensionContext, path: string) {
        if (w.discovery)
            w.pastRuns.push(await store!.put(w.discovery));
        w.discovery = newDiscovery(parseDataset(await readProjectJson(ctx.cwd, path)), w.settings.discoveryRounds);
        await save(ctx);
    }
    async function check(ctx: ExtensionContext, signal: AbortSignal, expression: string) {
        if (!w.discovery)
            throw new Error("Load a dataset to define the features and premises");
        ...
        if (proof.rho === 1 && empirical.counterexamples.length)
            throw new Error("Certificate contradicts exact examples; stop and inspect the environment");
        const evidence = { expression: expr, premises: d.premises, empirical, proof };
        const key = await store!.put(evidence);
        w.evidence.push(key);
        await save(ctx);
        ...
        async execute(_id, args, signal, _update, ctx) {
            ...
            return exclusive(ctx, async (combined) => {
                if (args.action === "start") {
                    if (w.proof)
                        throw new Error("A proof project already exists; use /math start to retain it and start another");
                    if (!args.problem)
                        throw new Error("A problem is required");
                    await replaceProof(ctx, args.problem, args.assumptions ?? []);
                }
                else
                    await proofWork(ctx, combined, args.action === "run");
                return result();
            }, signal);
        } });
    pi.registerTool({ name: "math_discover", label: "Math discovery", description: "Discover conjectures through weighted data, feature spotters, scaffolding, skepticism and independent proof feedback. JSON datasets must be inside the project. Output limit: 12,000 characters.", promptSnippet: "Explore conjectures and concepts from an explicit mathematical dataset",
        ...
        async execute(_id, args, signal, _update, ctx) {
            ...
            return exclusive(ctx, async (combined) => { if (args.action === "import") {
                if (w.discovery)
                    throw new Error("Use /math dataset to replace an existing environment");
                await importDataset(ctx, args.path ?? "");
            }
            else
                await discover(ctx, combined, args.action === "run"); return result(); }, signal);
        } });
    ...
    pi.registerTool({ name: "math_note", label: "Math knowledge", description: "Preserve a sourced lemma, failed approach, reference or observation with hypotheses and caveats. Entries are recorded as unverified; this tool cannot certify a claim.", promptSnippet: "Record reusable research knowledge with provenance",
        ...
        async execute(_id, args, signal, _update, ctx) { return exclusive(ctx, async () => { if (!w.proof)
            throw new Error("Start a proof project first"); addKnowledge(w.proof, KnowledgeSchema.parse({ ...args, id: `k-${randomUUID()}`, status: "unverified" })); await save(ctx); return result(); }, signal); } });
    pi.registerCommand("math", { description: "Human-guided mathematical research; /math help for commands", handler: async (args, ctx) => {
            ...
            try {
                ...
                if (command === "cancel") {
                    await stop();
                    show("Research cancelled; completed checkpoints are retained.");
                    return;
                }
                if (command === "status") {
                    await load(ctx);
                ...
                await exclusive(ctx, async (signal) => {
                    switch (command) {
                        case "start":
                            ...
                            await replaceProof(ctx, rest);
                            break;
                        case "step":
                        case "run":
                            await proofWork(ctx, signal, command === "run");
                            break;
                        case "approve":
                            if (!w.proof || !["awaiting-route", "awaiting-plan"].includes(w.proof.phase))
                                throw new Error("No route or plan awaits approval; final acceptance uses /math accept");
                            approve(w.proof, rest || "Approved by human command");
                            await save(ctx);
                            break;
                        case "accept":
                            if (w.proof?.phase !== "awaiting-acceptance")
                                throw new Error("A complete global review is required before acceptance");
                            approve(w.proof, rest);
                            await save(ctx);
                            break;
                        case "amend":
                            if (!w.proof)
                                throw new Error("No proof project");
                            amend(w.proof, rest, "Human changed the target");
                            await save(ctx);
                            break;
                        case "reexplore":
                            if (!w.proof)
                                throw new Error("No proof project");
                            reexplore(w.proof, rest);
                            await save(ctx);
                            break;
                        case "retry": {
                            const p = w.proof;
                            if (!p || p.phase !== "blocked" || !p.plan)
                                throw new Error("Retry applies to blocked local proof work; adjust round limits or re-explore otherwise");
                            humanDecision(p, "retry", rest);
                        ...
                        case "dataset":
                            await importDataset(ctx, rest);
                            break;
                        case "discover":
                            await discover(ctx, signal, rest === "run");
                            break;
                        case "use-conjecture": {
                            const d = w.discovery, last = d?.rounds.at(-1);
                            if (!d || !last)
                                throw new Error("No discovery conjecture");
                            await replaceProof(ctx, `For integer features ${d.dataset.features.join(", ")}, establish ${formatExpr(last.conjecture)}`, d.dataset.premises.map(formatExpr));
                            break;
                        }
                        case "premise": {
                            const d = w.discovery, last = d?.rounds.at(-1);
                            if (!d || !last || last.proof.rho !== 1)
                                throw new Error("Only a machine-checked last conjecture can be promoted");
                            if (!rest)
                                throw new Error("Explain why this should become background knowledge");
                            checkedFeedback(last.conjecture, last.premises, last.proof);
                            if (d.dataset.premises.some(p => canonical(p) === canonical(last.conjecture)))
                                throw new Error("Already a premise");
                            d.dataset.premises.push(last.conjecture);
                            parseDataset(d.dataset);
                            d.status = "active";
                            w.humanEvents.push({ action: "promote-premise", reason: rest, at: new Date().toISOString(), artifactHash: hash(last) });
                            await save(ctx);
                            break;
                        }
                        case "config":
                            if (!rest) {
                                show(JSON.stringify(w.settings, null, 2));
                                return;
                            }
                            w.settings = SettingsSchema.parse(await readProjectJson(ctx.cwd, rest));
                            if (w.proof)
                                w.proof.config = structuredClone(w.settings.inference);
                            await save(ctx);
                            break;
                        case "profile":
                            if (!["compact", "paper"].includes(rest))
                                throw new Error("Profiles: compact or paper");
                            w.settings.inference = ConfigSchema.parse({ ...w.settings.inference, widths: rest === "paper" ? [16, 8, 5, 1] : [4, 2, 1], sampleSize: rest === "paper" ? 5 : 3 });
                            if (w.proof)
                                w.proof.config = structuredClone(w.settings.inference);
                            await save(ctx);
                            break;
                        case "export": {
                            const dir = await exportWorkbench(store!, w, async (path, text) => withFileMutationQueue(path, () => writeFile(path, text, { flag: "wx", mode: 0o600 })));
                            show(`Research dossier exported to ${dir}`);
                            return;
                        }
