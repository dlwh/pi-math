...
export interface WorkerRequest {
  id: string; role: string; system: string; prompt: string;
...
export interface Worker { complete(request: WorkerRequest): Promise<WorkerResponse> }
export interface AuditRecord {
  id: string; role: string; system: string; prompt: string;
  startedAt: string; finishedAt: string;
  response: WorkerResponse | null; error: string | null;
}
...
class Semaphore {
  private active = 0;
  private waiting: { resolve: (release: () => void) => void; reject: (error: Error) => void; signal: AbortSignal; cancel: () => void }[] = [];
  ...
  async acquire(signal: AbortSignal): Promise<() => void> {
    ...
    return new Promise((resolve, reject) => {
      const item = { resolve, reject, signal, cancel: () => {
        this.waiting = this.waiting.filter(w => w !== item); reject(abortError());
      } };
  ...
  private release(): void {
    ...
    if (next) { next.signal.removeEventListener("abort", next.cancel); next.resolve(() => this.release()); }
    else this.active--;
  }
}
...
export class Broker {
  ...
  async ask<T>(role: string, instruction: string, input: unknown, schema: z.ZodType<T>): Promise<T> {
    ...
    if (prompt.length + system.length > this.config.maxInputChars) throw new BudgetError("Input exceeds configured context limit; narrow the research task or raise maxInputChars explicitly");
    ...
    try {
      ...
      if (this.usage.calls >= this.config.maxCalls || this.usage.reservedOutputTokens + this.config.maxOutputTokens > this.config.maxReservedOutputTokens) throw new BudgetError("Inference budget exhausted; inspect progress before starting another invocation");
      this.usage.calls++; this.usage.reservedOutputTokens += this.config.maxOutputTokens; reserved = true;
      ...
      throwIfAborted(signal);
      if (response.text.length > this.config.maxOutputTokens * 40) throw new Error("Worker output exceeds response size limit");
      if (response.usage) {
        const u = response.usage;
        if (![u.input,u.output,u.cost].every(n => Number.isFinite(n) && n >= 0)) throw new Error("Invalid provider usage");
        this.usage.input += u.input; this.usage.output += u.output; this.usage.reportedCost += u.cost;
      }
      ...
      try {
        if (reserved) await this.audit({id,role,system,prompt,startedAt,finishedAt:new Date().toISOString(),response,error:failure});
      } finally { release(); }
    }
  }
}
...
export function sample<T>(items: readonly T[], k: number, random: () => number): T[] {
  if (!Number.isInteger(k) || k < 1 || !items.length) throw new Error("Invalid sample");
  const copy = [...items];
  for (let i = copy.length - 1; i > 0; i--) {
    const r = random(); if (r < 0 || r >= 1 || !Number.isFinite(r)) throw new Error("Invalid random source");
    const j = Math.floor(r * (i + 1)); [copy[i],copy[j]] = [copy[j]!,copy[i]!];
  }
