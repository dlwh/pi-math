...
test("DAG rejects cycles, missing dependencies, duplicate IDs and disconnected results", () => {
  ...
  ]) { const p=structuredClone(diamond); mutate(p); assert.throws(()=>validatePlan(p)); }
...
test("repairs invalidate all descendants but preserve independent accepted work", () => {
  ...
  assert.equal(changed.a!.status,"accepted"); assert.equal(changed.c!.status,"accepted");
  assert.equal(changed.b!.status,"pending"); assert.equal(changed.d!.status,"pending");
  assert.deepEqual(frontier(diamond,changed),["b"]);
  const next=structuredClone(diamond); next.sections[0]!.task="Changed hypothesis";
...
test("immutable object storage detects corruption, rejects traversal and symlinks", async () => {
  ...
  try {
    const store=new ObjectStore(cwd); const key=await store.put({proof:"draft"});
    ...
    try { await symlink(cwd,join(other,".pi")); await assert.rejects(new ObjectStore(other).put({a:1}),/symlink/); }
