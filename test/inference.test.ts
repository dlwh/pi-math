...
test("concurrent requests share strict call reservations and a concurrency limit", async () => {
  ...
  assert.equal(broker.usage.calls,7); assert.equal(peak,2);
...
test("token reservation blocks before dispatch, including after malformed output", async () => {
  ...
  assert.equal(calls,1); assert.equal(broker.usage.calls,1);
});
...
test("cancellation rejects queued and active work without starting new calls", async () => {
  ...
  assert.ok(result.every(r=>r.status==="rejected"));assert.equal(calls,1);
});
test("independent uniform subsets are reproducible, internally distinct and can overlap", () => {
  const a=seededRandom(0),b=seededRandom(0); const population=Array.from({length:20},(_,i)=>i);
  const counts=new Map<number,number>();let overlap=false,prior:number[]=[];
  for(let i=0;i<2000;i++){
    const group=sample(population,5,a);assert.deepEqual(group,sample(population,5,b));assert.equal(new Set(group).size,5);
    if(group.some(x=>prior.includes(x)))overlap=true;prior=group;
    for(const x of group)counts.set(x,(counts.get(x)??0)+1);
  }
  assert.ok(overlap);assert.ok([...counts.values()].every(n=>n>400&&n<600));
});
test("synthesis cannot silently erase a minority fatal objection", async () => {
  const worker:Worker={async complete(r){
    ...
    if(r.role.endsWith("/generate"))return {text:JSON.stringify({value:input.candidate})};
    if(r.role.endsWith("/synthesize"))return {text:JSON.stringify({value:{value:99},resolutions:[]})};
...
test("discharging an objection requires a separate explicit reviewer disposition", async () => {
  const worker:Worker={async complete(r){const i=JSON.parse(r.prompt);
    if(r.role.endsWith("/generate"))return {text:'{"value":0}'};
    if(r.role.endsWith("/synthesize"))return {text:JSON.stringify({value:{value:1},resolutions:i.inheritedObjections.map((o:{id:string})=>({id:o.id,reason:"Replaced the erroneous claim"}))})};
    return {text:JSON.stringify({verdict:"accept",summary:"Checked",resolved:i.artifact.value===1?i.inheritedObjections.map((o:{id:string})=>({id:o.id,reason:"The replacement no longer asserts the false statement"})):[],issues:i.artifact.value===0?[{claim:"Bad claim",reason:"Concrete defect",severity:"major",scope:"artifact",sections:[]}]:[]})};
  }};
  const runner=new StageRunner(new Broker(worker,ConfigSchema.parse({widths:[1,1]})));
  const root=await runner.run("test","Test",{},output);
  assert.equal(root.objections.length,0);assert.equal(root.reviews[0]!.review.resolved.length,1);
});
