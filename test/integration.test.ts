...
async function harness(){
  ...
  let branch:BranchEntry[]=[];const messages:string[]=[],notifications:string[]=[];
  ...
  async function emit(event:string){for(const h of extension.handlers.get(event)??[])await h({type:event},ctx);}
...
test("published Pi loader registers four tools and human-only commands without starting background work",async()=>{
  const h=await harness();try{
    ...
    assert.ok(h.extension.commands.has("math"));assert.equal(h.getBranch().length,0);
    const schema=h.extension.tools.get("math_research")!.definition.parameters;
    assert.equal(Check(schema,{action:"approve"}),false);assert.equal(Check(schema,{action:"run"}),true);
    await h.command("help");assert.match(h.messages.at(-1)!,/\/math approve/);assert.deepEqual(h.notifications,[]);
  }finally{await h.cleanup();}
});
test("real extension commands and tools restore the selected session branch",async()=>{
  const h=await harness();try{
    await h.emit("session_start");await h.command("start Original question");const first=h.getBranch();
    await h.command("start Different question");assert.equal((await h.tool("math_research",{action:"status"})).content[0]!.type,"text");
    await h.emit("session_before_tree");h.setBranch(first);await h.emit("session_tree");await h.command("status");
    assert.match(h.messages.at(-1)!,/Original question/);assert.doesNotMatch(h.messages.at(-1)!,/Different question/);
...
test("parallel tools cannot race even during cold initialization",async()=>{
  const h=await harness();try{
    ...
    const restored=await restoreWorkbench(new ObjectStore(h.cwd),h.getBranch());assert.equal(restored.proof!.problem,"First");
  }finally{await h.cleanup();}
});
test("Pi adapter isolates context, respects role/scoped models, disables retries and rejects incomplete responses",async()=>{
  ...
  assert.equal((await worker.complete(request)).text,'{"ok":true}');assert.equal(context!.messages.length,1);assert.deepEqual(context!.tools,[]);assert.equal(context!.systemPrompt,"Specific task");
  assert.equal(options!.maxTokens,1024);assert.equal(options!.maxRetries,0);assert.equal(options!.signal,request.signal);
  response.stopReason="length";await assert.rejects(worker.complete(request),/length/);response.stopReason="stop";
  const restricted=new PiWorker({model,modelRegistry:registry,scopedModels:[{model:{...model,id:"another"}}]},ConfigSchema.parse({}));await assert.rejects(restricted.complete(request),/scoped/);
});
test("workbench exports contain portable model records, certificates and the original research state",async()=>{
  const dir=await mkdtemp(join(tmpdir(),"pi-math-export-"));try{
    const store=new ObjectStore(dir),w=emptyWorkbench();w.proof=newResearch("Test target");w.audit.push(await store.put({role:"fixture",response:"recorded"}));
    const path=await exportWorkbench(store,w);const output=JSON.parse(await readFile(join(path,"research.json"),"utf8"));
    assert.equal(output.audit[0].value.response,"recorded");assert.equal(output.state.proof.problem,"Test target");
    assert.match(await readFile(join(path,"research.md"),"utf8"),/No final human acceptance/);assert.match(await readFile(join(path,"proof.tex"),"utf8"),/\\documentclass/);
    await assert.rejects(readProjectJson(dir,"../missing"));await writeFile(join(dir,"bad.json"),"no");await assert.rejects(readProjectJson(dir,"bad.json"));
...
test("bounded symbolic policy runs without a model and survives checkpoint validation",async()=>{
  ...
  const w=emptyWorkbench();w.discovery=newDiscovery(d,1);await new DiscoveryEngine(w.discovery,new HeuristicPolicy(),new AlgebraProver()).step();
  assert.equal(w.discovery.rounds.length,1);assert.equal(parseWorkbench(w).discovery!.rounds.length,1);
});
